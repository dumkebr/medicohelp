...
// Clarice brain (v6) - loads kb/index.json then each file listed there.
// Keeps button actions, logging, export, and adds quick-chips support.
export async function loadAllKB(base = './kb/') {
  try {
    ...
    for (const f of files) {
      ...
      } catch (e) { console.warn('Falha ao carregar KB file', f, e); }
    }
    return all;
  } catch (e) {
    console.warn('Erro loadAllKB', e);
    return [];
  }
}
