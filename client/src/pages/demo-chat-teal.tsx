...
export default function DemoChatTeal() {
  ...
  async function send() {
    ...
    setTimeout(() => {
      setMessages(m => [...m, {
        who: "ai",
        text: "Entendi. Vamos analisar esse caso clínico. Pode me passar mais detalhes sobre sinais vitais e exame físico?"
      }]);
    }, 800);
  }
    ...
    <div>
      ...
      <div className="container grid">
        ...
        <section className="chat card">
          <div className="msgs">
            {messages.map((m, i) => (
              <div key={i} className={"bubble " + (m.who === "user" ? "user" : "ai")}>
                {m.text}
              </div>
            ))}
          </div>
          <div className="inputbar">
            ...
            />
            <button className="btn btn-primary" onClick={send} data-testid="button-send">
              Enviar
            </button>
          </div>
        </section>
